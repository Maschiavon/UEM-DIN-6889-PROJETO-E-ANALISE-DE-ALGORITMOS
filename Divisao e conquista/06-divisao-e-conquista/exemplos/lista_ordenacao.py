...
def main():
    ...
    def build(A: List[int], n: int) -> Lista:
        def iter(lst: Lista, n: int) -> Lista:
            if n == 0:
                return lst
            else:
                return iter(Link(A[n - 1], lst), n - 1)
        return iter(None, n)
    ...
    if caso == 'crescente':
        plt.title("Tempo de execução para listas com elementos em ordem crescente")
    elif caso == 'aleatorio':
        plt.title("Tempo de execução para listas com elementos em ordem aleatória")
    else:
        plt.title("Tempo de execução para listas com elementos em ordem decrescente")
    plt.savefig(caso + '.pdf')
    plt.show()
